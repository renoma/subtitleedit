...
namespace Nikse.SubtitleEdit.Forms.Ocr
{
    ...
    public sealed partial class VobSubOcr : PositionAndSizeForm, IBinaryParagraphList
    {
        ...
        private static readonly Color _listViewRed = Color.IndianRed;
        ...
        private string SplitAndOcrBinaryImageCompare(Bitmap bitmap, int listViewIndex)
        {
            ...
            {
                subtitleListView1.SetBackgroundColor(listViewIndex, _listViewGreen);
            }
            return line;
        }
        ...
        private string FixNocrHardcodedStuff(string input)
        {
            ...
            {
                string lastLastLine = null;
                var lastLastP = _subtitle.GetParagraphOrDefault(_selectedIndex - 2);
                if (lastLastP != null && !string.IsNullOrEmpty(lastLastP.Text))
                {
                    lastLastLine = lastLastP.Text;
                }
        ...
        private void VobSubOcr_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            // Alt + ↓
            else if ((e.KeyData == UiUtil.GetKeys(Configuration.Settings.Shortcuts.GeneralGoToNextSubtitlePlayTranslate) || (e.KeyCode == Keys.Down && e.Modifiers == Keys.Alt)))
            {
                int selectedIndex = 0;
                if (subtitleListView1.SelectedItems.Count > 0)
                {
